...
mora sadržavati identičnu kopiju kompletnog sadržaja baze podataka i konstantno se sinkronizirati sa susjednim čvorovima. \\
...
Čvorovi koji primaju novu transakciju provjeravaju njenu valjanost i uključuju ju u idući blok. Prije nego što se novi blok transakcija poveže u postojeći lanac
...
i procesorsku moć. U ovom algoritmu nove blokove ne stvaraju rudari nego kovači (engl. \emph{forgers}). \\ Umjesto da se pouzdanost \emph{blockchaina} oslanja
...
Dodatno, ako validatori ne izvršavaju svoje dužnosti i validiraju neispravne transakcije za to bivaju kažnjavani oduzimanjem uloženih tokena. Vrijednost uloženih tokena je
...
Pametni ugovori su dobra osnova za izgradnju složenog sustava, no problem je što interakcija s ugovorima nije prilagođena široj javnosti. Kako bi se taj problem smanjio
...
Danas postoje i druge igre na \emph{Ethereum} mreži koje iskorištavaju mogućnost razmijene dobara unutar igre za tokene. Na stranici
 \emph{dappradar}\footnote{\url{https://dappradar.com/rankings/protocol/eth/category/games}} moguće je vidjeti sortirane pametne ugovore po broju izvršenih transakcija
 nekom razdoblju. U kategoriji igara, trenutno najpopularnije po broju igrača su \emph{My Crypto Heroes}\footnote{\url{https://www.mycryptoheroes.net/}},
  \emph{BRAVE FRONTIER HEROES}\footnote{\url{https://bravefrontierheroes.com/}} i \emph{CryptoSpells}\footnote{\url{https://en.cryptospells.jp/}}.

\chapter{Ethereum mreža na raznorodnom sklopovlju}
...
S obzirom na to da je Cortex-A7 ARM procesor postoje razne mogućnosti kad je u pitanju odabir operativnog sustava. Tvornički dolazi
...
Novi blok se generira svakih 15 sekundi jer je tako zadano prilikom inicijalizacije \emph{genesis.json} datotekom. \\
...
\begin{sazetak}
U ovom radu su istražene osnove tehnologije distribuirane glavne knjige i najpoznatiji algoritmi konsenzusa koji mijenjaju centralni autoritet.
Uspoređene su njihove prednosti i mane. U nastavku je uvod u \emph{Ethereum} protokol i njegovu arhitekturu. Objašnjen je način provođenja transakcija i izvršavanja
programskog koda na \emph{Ethereum} virtualnom stroju. Najveći dio rada je usmjeren postavljanju privatne \emph{Ethereum} mreže između čvorova raznorodnog sklopovlja.
Detaljno je opisan postupak izrade novčanika, provođenja transakcija, postavljanja i izvršavanja programskog koda pametnog ugovora na \emph{blockchain}. 
Ispitani su zahtjevi procesa zaduženog za sudjelovanje u mreži na svakom čvoru.
...
\begin{abstract}
In this paper, basics of the distributed ledger technology are explored with well-known consensus algorithms that are used to replace central authority.
Their advantages and disadvantages are compared. In the next part is the introduction to the \emph{Ethereum} protocol and its architecture. Execution of program code
and transactions on \emph{Ethereum} virtual machine is explained. The biggest part of this paper is focused on setting up a private \emph{Ethereum} network between nodes
powered by the heterogeneous hardware. Procedure of creating wallets, executing transactions, deployment and execution of the smart contract program code is explained in details.
Process requirements used for participating in the network are tested on every node. 
